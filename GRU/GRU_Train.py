...
for l in layer_lst:
    ...
    hidden_range = range(len(hidden_lst))
    if l == 3:
        hidden_range = range(3, len(hidden_lst))
    for h in hidden_range:
        
        for m_s in range(model_sample):
            
            print(f'{l}layer, {h}hidden, {m_s}model')
           
            ###################################### Hyper Parameter ######################################
            
            input_size = x_seq.size(2)
            num_layers = l
            hidden_size = hidden_lst[h]
            ...
            for epoch in range(num_epochs):
                ...
                if best < r2:
                    best = r2
                    torch.save(model.state_dict(), f'GRU_{l}layer_{h}hidden_{m_s}sample')
                    cnt = 0
                else:
                    cnt = cnt + 1
                ...
                if epoch%50 == 0:
                    ...
                    print(f'epoch: {epoch} / layer: {l} / hidden: {h} / sample: {m_s}')
                    print(f'Best R2: {best:.4f} / Test rmse: {rmse:.4f} / R2: {r2:.4f}')
